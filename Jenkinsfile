pipeline {
    ...
    environment {
        IMAGE_NAME = "yash5090/wnste-arch"
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Arch-Wbste-Deployment.git'
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop wnste-arch | true"
                sh "docker rm wnste-arch | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh "docker run -d --name wnste-arch -p 5000:80 ${IMAGE_NAME}:${TAG}" 
                //}
            }
        }
        ...
        stage('Switch Traffic Between Blue & Green Environment') {
            ...
            steps {
                script {
                    ...
                    withKubeConfig(caCertificate: '', clusterName: '', contextName: '', credentialsId: 'k8s', namespace: '', restrictKubeConfigAccess: false, serverUrl: '') {
                        sh '''
                            kubectl patch service wnste-arch-service -p "{\\"spec\\": {\\"selector\\": {\\"app\\": \\"wnste-arch\\", \\"version\\": \\"''' + newEnv + '''\\"}}}"
                        '''
                    }
